...
namespace Chat.Infrastructure.Services.HttpClients
{
    public class IdentityService : IIdentityService
    {
        ...
        public async Task<User> GetUserDetailsAsync(string userId, CancellationToken cancellationToken = default)
        {
            try
            {
                var response = await _client.GetAsync($"User/GetUserDetails/{userId}", cancellationToken: cancellationToken);
